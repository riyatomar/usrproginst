...
def process_relation(output):
    ...
    if CC_exists:
            # Collect all CC indices while ensuring rows have enough elements
            CC_indices = [row[0] for row in output if len(row) > 3 and row[3] == 'CC']

            # Loop through each CC and process its associated rows
            for CC_index in CC_indices:
                CC_dep = output[CC_index - 1][7]  # Ensure index is valid
                CC_ptr_index = output[CC_index - 1][6]

                for row in output:
                    try:
                        row_ptr_index = row[6]  # Ensure index is valid
                        row_dep = row[7]  # Ensure index is valid
                        
                        # Update rows that depend on this particular CC
